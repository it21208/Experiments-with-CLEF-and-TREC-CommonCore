...
pathToQRELS = '/home/pfb16181/NetBeansProjects/lucene4ir-master/data/TREC-CommonCore/307-690.qrels'


def readInitialRanking(pathToQRELS):
    # read qrels file
    f = open(pathToQRELS, "r")
    topic_len_qrels_dict = {}
    listOfDocsForTopic = []
    previous_topic = ''
    for idx, line in enumerate(f):
        ...
        if idx != 1:
            if topic == previous_topic:
                ...
                if previous_topic not in topic_len_qrels_dict.keys():
                    topic_len_qrels_dict[previous_topic] = listOfDocsForTopic
    ...
    # for the last topic 
    topic_len_qrels_dict[topic] = listOfDocsForTopic
    return(topic_len_qrels_dict)
...
dict_of_len_ids_for_topic = readInitialRanking(pathToQRELS)

for query in queries:
    dictForIndexing = {}
    # Use BERT for mapping tokens to embeddings
    word_embedding_model = models.Transformer('bert-base-uncased')
    # Apply mean pooling to get one fixed s0ized sentence vector
...
dirToWrite = '/home/pfb16181/NetBeansProjects/hedwig/scripts/resultsFSSVmeanPoolingCommonCore.txt'
