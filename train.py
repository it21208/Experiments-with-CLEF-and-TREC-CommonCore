...
def train(args):
    ...
    else: # May load local file or download from huggingface
    ...
    train_dataset = DataGenerator(args.data_path, args.collection, args.batch_size, tokenizer, 'train',  args.device)
    validate_dataset = DataGenerator(args.data_path, args.collection, args.batch_size, tokenizer, 'dev', args.device)
    optimizer = init_optimizer(model, args.learning_rate, args.warmup_proportion, args.num_train_epochs, args.batch_size)
    model.train()
    best_score = 0
    step = 0
